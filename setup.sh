...
if ! sudo -H -u "$TARGET_USER" bash -c "export RBENV_ROOT=\"$TARGET_HOME/.rbenv\" && export PATH=\"$TARGET_HOME/.rbenv/bin:\$PATH\" && $TARGET_HOME/.rbenv/bin/rbenv versions 2>/dev/null | grep -q \"$RUBY_TARGET_VERSION\""; then
    console_output "Installing Ruby $RUBY_TARGET_VERSION (this may take several minutes)..."
    sudo -H -u "$TARGET_USER" bash -c "export RBENV_ROOT=\"$TARGET_HOME/.rbenv\" && export PATH=\"$TARGET_HOME/.rbenv/bin:\$PATH\" && $TARGET_HOME/.rbenv/bin/rbenv install $RUBY_TARGET_VERSION"
else
    console_output "Ruby $RUBY_TARGET_VERSION already installed"
fi

# Set global Ruby version
sudo -H -u "$TARGET_USER" bash -c "export RBENV_ROOT=\"$TARGET_HOME/.rbenv\" && export PATH=\"$TARGET_HOME/.rbenv/bin:\$PATH\" && $TARGET_HOME/.rbenv/bin/rbenv global $RUBY_TARGET_VERSION"
sudo -H -u "$TARGET_USER" bash -c "export RBENV_ROOT=\"$TARGET_HOME/.rbenv\" && export PATH=\"$TARGET_HOME/.rbenv/bin:\$PATH\" && $TARGET_HOME/.rbenv/bin/rbenv rehash"

# Verify Ruby installation
RUBY_VERSION=$(sudo -H -u "$TARGET_USER" bash -c "export RBENV_ROOT=\"$TARGET_HOME/.rbenv\" && export PATH=\"$TARGET_HOME/.rbenv/bin:$TARGET_HOME/.rbenv/shims:\$PATH\" && ruby -v" 2>/dev/null || echo "Ruby installation failed")
...
if echo "$RUBY_VERSION" | grep -q "ruby $RUBY_TARGET_VERSION"; then
    sudo -H -u "$TARGET_USER" bash -c "export RBENV_ROOT=\"$TARGET_HOME/.rbenv\" && export PATH=\"$TARGET_HOME/.rbenv/bin:$TARGET_HOME/.rbenv/shims:\$PATH\" && gem install ruby-lsp" || console_output "Failed to install ruby-lsp"
    sudo -H -u "$TARGET_USER" bash -c "export RBENV_ROOT=\"$TARGET_HOME/.rbenv\" && export PATH=\"$TARGET_HOME/.rbenv/bin:$TARGET_HOME/.rbenv/shims:\$PATH\" && gem install rubocop" || console_output "Failed to install rubocop"
else
